<template>
  <div class="min-h-screen w-full bg-gradient-to-br from-black via-[#181a1a] to-[#232323] px-4 py-10 flex flex-col items-center">
    <div class="max-w-lg w-full mx-auto mb-8 bg-[#181b1c] border border-green-700 rounded-2xl shadow-lg p-7 flex flex-col gap-5">
      <label for="busqueda" class="block mb-1 text-green-400 font-bold text-lg">Buscar materia o examen:</label>
      <input
        id="busqueda"
        type="text"
        v-model="busqueda"
        class="w-full py-3 px-4 rounded-lg bg-[#16191a] border border-gray-700 text-green-200 font-semibold text-lg transition focus:ring-2 focus:ring-green-500 focus:outline-none"
      ...
      <select
        ...
        class="w-full py-3 px-4 rounded-lg bg-[#16191a] border border-gray-700 text-green-300 font-semibold text-lg transition focus:ring-2 focus:ring-green-500 focus:outline-none"
      >
        <option v-for="mat in materiasFiltradas" :key="mat" :value="mat">{{ materiasNombres[mat] || mat }}</option>
      </select>
      <button
        @click="recargarResultados"
        class="mt-4 py-2 px-4 bg-green-700 hover:bg-green-800 rounded-lg font-bold text-white shadow transition whitespace-nowrap"
      >
        Recargar Resultados
      </button>
    </div>
    ...
    <div class="w-full max-w-5xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 justify-center items-center">
      ...
      <template v-else>
        <div
          ...
          :key="card.id"
          class="bg-gradient-to-br from-[#191f1f] to-[#242c23] border border-gray-700 rounded-2xl shadow-xl flex flex-col justify-between p-8 min-h-[260px]
                 hover:scale-105 hover:border-green-500 transition-transform duration-180 mx-auto w-full max-w-sm animate__animated animate__fadeIn"
        >
          ...
          <div v-if="resultadosExamenes[card.id]" class="mb-4 text-green-200 text-center space-y-1">
            ...
            <div class="relative w-full h-4 bg-gray-900 rounded-full overflow-hidden mt-3 shadow-inner" role="progressbar" :aria-valuenow="resultadosExamenes[card.id].puntaje" aria-valuemin="0" aria-valuemax="100">
              <div
                class="h-full bg-gradient-to-r from-green-400 to-green-700 rounded-full transition-all duration-700"
          ...
          <div v-else class="mb-4 text-green-600 text-center italic font-semibold">
            Sin intento registrado
          </div>
          <div class="flex justify-between items-center mt-auto gap-4">
            <button
              class="flex-1 py-3 rounded-lg bg-green-700 text-white font-bold shadow
                     hover:bg-green-600 transition focus:outline-none focus:ring-2 focus:ring-green-600"
            ...
            <button 
              ...
              :class="['px-4 py-2 rounded-lg font-bold transition shadow', favoritos.has(card.id) ? 'bg-green-500 text-black' : 'bg-gray-800 hover:bg-green-900 text-green-300']"
            >
              {{ favoritos.has(card.id) ? '★ Favorito' : '☆ Favorito' }}
            </button>
          </div>
        </div>
        <div v-if="!examenesFiltrados.length" class="col-span-full text-red-400 text-center font-bold text-lg py-20">
...
const materiasFiltradas = computed(() => {
  const unique = new Set(examenes.map(e => e.materia));
...
const estadoClass = id => {
  if (!resultadosExamenes.value[id]) return "text-red-400";
  if (resultadosExamenes.value[id].puntaje >= 75) return "text-green-400";
