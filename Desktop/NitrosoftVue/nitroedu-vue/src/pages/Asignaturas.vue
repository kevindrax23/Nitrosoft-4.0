<template>
  <div class="min-h-screen font-sans px-2 py-6 bg-gradient-to-br from-black via-[#191e1f] to-[#232323]">
    <div class="w-full max-w-7xl mx-auto">
      <div class="mb-8 flex flex-col md:flex-row gap-6 items-center">
        <input
          v-model="busqueda"
          placeholder="Buscar tema, concepto o ejemplo..."
          class="flex-1 py-3 px-6 rounded-lg bg-[#161b1c] border border-gray-700 text-green-300 font-semibold text-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition"
        />
        <button
          class="px-6 py-3 rounded-lg bg-gradient-to-r from-green-600 to-green-700 text-white font-bold shadow hover:from-green-800 hover:to-green-600 transition animate__animated animate__bounce"
          @click="resetBusqueda"
        >
          Limpiar búsqueda
        </button>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-7 md:gap-10">
        <div
          ...
          :key="tema.id"
          class="bg-gradient-to-br from-[#191f1f] to-[#232b23] border border-gray-800 rounded-3xl shadow-xl px-8 py-8 flex flex-col gap-5
                backdrop-blur group hover:scale-105 hover:shadow-green-700 transition-all duration-180 animate__animated animate__fadeInUp"
        >
          <div class="flex justify-between items-center mb-2">
            <h3 class="text-2xl font-extrabold text-green-400 text-center w-full">{{ tema.titulo }}</h3>
            <button @click="toggleFavorito(tema)" class="text-xl ml-2">
              <span :class="[tema.favorito ? 'text-green-300' : 'text-gray-700']" class="transition">
                {{ tema.favorito ? '★' : '☆' }}
              </span>
            </button>
          </div>
          <p class="mb-1 text-gray-300 text-center">{{ tema.descripcion }}</p>
          <!-- Barra de progreso minimalista -->
          <div class="mt-1 mb-2">
            <div class="text-green-200 text-xs mb-1 text-center">Progreso: {{ tema.progreso }}%</div>
            <div class="h-2 w-full bg-gray-900 rounded overflow-hidden shadow">
              <div :style="{ width: tema.progreso + '%' }" class="bg-gradient-to-r from-green-500 to-green-700 h-2 transition-all duration-400"></div>
            </div>
          </div>
          <!-- Video -->
          <div class="relative w-full overflow-hidden pt-[56.25%] rounded-lg mb-2 ring-1 ring-gray-800 shadow">
            <iframe
              :src="tema.videoUrl"
              frameborder="0"
              allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen
              class="absolute top-0 left-0 w-full h-full rounded-lg bg-black"
              :title="'Video de ' + tema.titulo"
            ></iframe>
          </div>
          <!-- Ejercicios -->
          <ul class="list-disc pl-6 mb-2 text-green-200 text-[15px] text-left">
            <li v-for="ej in tema.ejercicios" :key="ej">
              <span>{{ ej }}</span>
              <button @click="responderEjercicio(tema, ej)" class="ml-2 px-3 py-1 text-xs font-bold bg-green-800 hover:bg-green-600 rounded text-green-100 transition">
                Resolver
              </button>
            </li>
          </ul>
          <div class="flex gap-2 justify-center">
            <button
              class="px-6 py-2 rounded-lg bg-green-700 text-white font-bold shadow hover:bg-green-600 transition focus:outline-none focus:ring-2 focus:ring-green-600"
              @click="accederTema(tema)"
            >
              Acceder
            </button>
            <button
              class="px-6 py-2 rounded-lg bg-gray-800 text-green-300 font-bold border border-green-700 hover:bg-gray-900 hover:text-green-400 transition focus:outline-none focus:ring-1 focus:ring-green-700"
      ...
      <div v-if="modalEjercicio" class="fixed inset-0 bg-black bg-opacity-90 flex justify-center items-center z-50 px-4">
        <div class="bg-[#161b1c] rounded-3xl p-8 max-w-md w-full border border-green-800 shadow-2xl animate__animated animate__fadeIn">
          <h3 class="text-green-400 text-xl text-center font-extrabold mb-4">{{ ejercicioActivo.titulo }}</h3>
          <div class="text-green-200 mb-4 font-semibold">{{ ejercicioActivo.ej }}</div>
          <input v-model="respuestaEjercicio" placeholder="Tu respuesta..." class="w-full px-4 py-2 rounded-lg bg-black border border-green-700 text-green-200 mb-4" />
          <div class="flex gap-2 mt-2">
            ...
            <button @click="cerrarEjercicio" class="px-4 py-2 bg-gray-800 rounded font-bold text-green-100 hover:bg-gray-900 transition">Cancelar</button>
          </div>
          <div v-if="ejercicioFeedback" class="mt-3 text-green-400 text-center">{{ ejercicioFeedback }}</div>
        </div>
      </div>
      <!-- Modal detalles -->
      <div v-if="modalDetalles" class="fixed inset-0 bg-black bg-opacity-90 flex justify-center items-center z-50 px-4">
        <div class="bg-[#1a1e1d] rounded-3xl p-8 max-w-md w-full border border-green-800 shadow-2xl animate__animated animate__fadeIn">
          <h3 class="text-green-400 text-xl text-center font-extrabold mb-4">{{ detallesActivo.titulo }}</h3>
